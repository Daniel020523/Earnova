...
create table if not exists public.jobs (
  ...
  salary_range text,
  summary text,              -- short one-liner shown on the closed job card
  requirements text not null,     -- what the person must have (skills, experience, tools)
  responsibilities text not null, -- what they'll actually be doing
  work_schedule text not null,    -- hours / days / shift pattern
  rules text,                     -- targets/conditions, e.g. "Bring in 20 new customers/month to secure salary"
  apply_link text,
  apply_email text,
...
-- If you ran an earlier version of this schema that had a single
-- "description" column, uncomment the lines below to migrate:
...
-- alter table public.jobs add column if not exists work_schedule text;
-- alter table public.jobs add column if not exists rules text;
-- alter table public.jobs add column if not exists summary text;
-- update public.jobs set requirements = coalesce(requirements, ''), responsibilities = coalesce(responsibilities, description), work_schedule = coalesce(work_schedule, '') where requirements is null or responsibilities is null;
...
-- alter table public.jobs alter column work_schedule set not null;
