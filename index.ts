...
//
// CHANGE: commissions are now marked "completed" the moment a payment is
// verified — no manual admin approval step. If you ever need to claw back
// a disputed/charged-back sale, flip that row's status to "rejected" by
// hand (or from a Paystack refund webhook, if you add one later).
...
serve(async (req) => {
  ...
  try {
    ...
    // 7. Record the sale. Commission is marked "completed" immediately on a
    //    verified payment — no pending/admin-approval step anymore.
